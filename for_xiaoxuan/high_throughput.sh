...
ls lig_folder1/*.pdbqt | sort -n -t g -k 2 > tests.txt

# Still use MR1
receptor=$(echo prot.pdbqt)
...
grep CA prot.pdb | awk '$6==48 {print $7}' > x_coor.txt
grep CA prot.pdb | awk '$6==48 {print $8}' > y_coor.txt
grep CA prot.pdb | awk '$6==48 {print $9}' > z_coor.txt
...
# and now actually run vina with these...
    # THIS LINE ONLY WORKS IF RUNNING ON CROOKS COMPUTER.
    # REPLACE WITH YOUR LOCAL BUILD OF VINA IF NECESSARY
    /home/erin/Desktop/vina_apps/vina --config config.txt --log log$z.txt
